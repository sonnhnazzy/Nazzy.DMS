package vn.nazzy.dms.UpdateCheckerer;
...
public class UpdateChecker {
    ...
    public void checkForUpdateByVersionCode(String url) {
        if (isOnline()) {
            if (haveValidContext) {
                ...
                if (versionCode >= 0) {
                    try {
                        ...
                        // Log.e(TAG, "Khong convert duoc du lieu");
                    }
            ...
            if (useToasts) {
                makeToastFromString(
                        "Không kết nối Internet, đề nghị xem lại...").show();
            }
        }
    }
    ...
    public void launchMarketDetails() {
        if (haveValidContext) {
            if (hasGooglePlayInstalled()) {
                ...
                if (useToasts) {
                    makeToastFromString(
                            "Google Play không thể cài đặt ứng dụng vào thiết bị của bạn...")
                            .show();
                }
            }
        }
